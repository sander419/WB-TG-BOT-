...
- Слой данных проверен на живой базе: `npm run smoke:db` создаёт временную
  организацию, наполняет её и прогоняет выборки, сводку, диагностику и алерты.
  Воркер вживую не гонялся — для него нужен боевой токен площадки.
...
| `npm test` | Тесты ядра и нормализации (`node:test`, без раннера) |
